...
const getAllcolorPallets = async (req, res) => {
  try {
    /**
     * Retriving all the documents/data from collection which is in database.
     * find({}) -> This is mongoose method and it will find/retrive all the documents as the method has no parameter.
     */
...
const getGradientColors = async (req, res) => {
  try {
    ...
    res.status(200).json(gradientColorData);
  } catch (error) {
    res.status(500).json({ error: "Internal Server Error" });
  }
};
...
const getSolidColors = async (req, res) => {
  try {
    ...
    res.status(200).json(solidColorData);
  } catch (error) {
    res.status(500).json({ error: "Internal Server Error" });
  }
};
